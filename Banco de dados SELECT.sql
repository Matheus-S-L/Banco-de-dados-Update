...
INSERT INTO tb_bairros (nm_bairro, id_cidade, sg_estado) VALUES ('AEROPORTO', 4, 'RJ');
...
INSERT INTO tb_vendedores (nm_vendedor, nm_endereco, ds_email) VALUES ('OUTRO VENDEDOR', 'RUA TESTE 123', 'VENDEDOR4@TESTE.COM');
...
-- 2. CDIMOVEL, VLPRECO e VLPRECO com 10% de aumento
SELECT
    cd_imovel,
    vl_preco,
    vl_preco * 1.10 AS vl_preco_aumento
FROM tb_imoveis;

-- 3. Igual anterior + diferença
SELECT
    cd_imovel,
    vl_preco,
    vl_preco * 1.10 AS vl_preco_aumento,
    (vl_preco * 1.10) - vl_preco AS diferenca
FROM tb_imoveis;

-- 4. Nome vendedor maiúsculo e email minúsculo
SELECT
    UPPER(nm_vendedor) AS nm_vendedor,
    LOWER(ds_email_vendedor) AS email
FROM tb_vendedores;

-- 5. Nome comprador + cidade em uma coluna
SELECT
    CONCAT(nm_comprador, ' - ', nm_cidade_comprador) AS comprador_cidade
FROM tb_compradores;

-- 6. Compradores com letra A no nome
SELECT *
FROM tb_compradores
WHERE nm_comprador LIKE '%A%';

-- 7. Primeira letra do nome + bairro
SELECT
    LEFT(nm_comprador, 1) AS inicial,
    nm_bairro_comprador
FROM tb_compradores;

-- 8. Código do imóvel e dias desde o lançamento
SELECT
    cd_imovel,
    DATEDIFF(CURRENT_DATE(), dt_lancto) AS dias_desde_lancamento
FROM tb_imoveis;

-- 9. Código do imóvel, data lançamento e dias
SELECT
    cd_imovel,
    dt_lancto,
    DATEDIFF(CURRENT_DATE(), dt_lancto) AS dias
FROM tb_imoveis;

-- 10. Igual anterior + data 15 dias após lançamento
SELECT
    cd_imovel,
    dt_lancto,
    DATEDIFF(CURRENT_DATE(), dt_lancto) AS dias,
    DATE_ADD(dt_lancto, INTERVAL 15 DAY) AS mais_15_dias
FROM tb_imoveis;